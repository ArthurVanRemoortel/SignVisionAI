    ...
    with (MediapipeHandLandmarker.create_from_options(hands_options) as hands_landmarker,
        ...
        while cap.isOpened():
            ...
            if hand_landmarker_result.hand_landmarks:
                ...
                left_angle = process_hand_orientation([gesture_frame.LEFT])[0]
                right_angle = process_hand_orientation([gesture_frame.RIGHT])[0]
                # print(right_sin)
                cv2.putText(frame, f"L: {left_angle[0]:.2f}",
                            (20, 20), cv2.FONT_HERSHEY_DUPLEX,
                            FONT_SIZE, HANDEDNESS_TEXT_COLOR, FONT_THICKNESS, cv2.LINE_AA)
                cv2.putText(frame, f"R: {right_angle[0]:.2f}",
